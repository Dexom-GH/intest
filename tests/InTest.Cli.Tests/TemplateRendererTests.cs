...
public class TemplateRendererTests
{
    ...
    // --- Hostile spec text (escaping into C# string literals) ---
    //
    // TestPlanBuilder.cs's fixture-filename check (FixtureDocument.TryValidateOperationKey)
    // only runs when NeedsFixture is true, because only then does the operationId become a
    // fixture file name. A parameterless, body-free operation never needs a fixture, so a `"`
    // or `\` in its operationId reaches TemplateRenderer completely unvalidated — the live bug
    // this escaping fixes. These plans all default NeedsFixture to true (the record's default,
    // matching every plan above), which is irrelevant to TemplateRenderer: rendering never
    // consults NeedsFixture itself, only the case Role — so hostile text is exercised directly
    // here without needing to fake the fixture-need computation the real bug depends on. The
    // actual "does this reach the renderer through the real fixture-need gate" proof lives in
    // CompileVerificationTests.GeneratedProjectWithHostileSpecTextCompiles, which compiles a
    // real generated project from a real spec — a string assertion here can only prove escaping
    // happened, not that the result is valid C#.
    ...
    {
        var rendered = Render(PlanAuth(403, IdentitySlot.Secondary, requiredScopes: ["orders.write\"x\\y"]));
    ...
    {
        // The most important regression guard for this change: escaping must be a complete
        // no-op for every operationId, display name, path template and schema key that
        // contains neither '"' nor '\' — otherwise this change would silently rewrite the
        // entire golden file's ordinary-text output (GoldenFileTests.OutputMatchesTheGoldenFile
        // is the byte-exact, whole-file version of this same guarantee).
